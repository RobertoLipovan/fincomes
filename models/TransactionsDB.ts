...
import { NOTION_API_TOKEN, TRANSACTIONS_DB_ID } from "@env";

class TransactionsDB {
  private notion: Client;
  private transactionsDbId: string;

  constructor() {
    this.notion = new Client({ auth: NOTION_API_TOKEN });
    this.transactionsDbId = TRANSACTIONS_DB_ID || "";
  }

  // Método para crear un registro
  async create(properties: any) {
    const response = await this.notion.pages.create({
      parent: { database_id: this.transactionsDbId },
      properties,
    });
    return response;
  }
